;;; packages.el --- athenacle-markdown layer packages file for Spacemacs.
;;
...
(defconst athenacle-markdown-packages
  '(
    edit-server
    flymd
    markdown-mode
  ...
  "The list of Lisp packages required by the athenacle-markdown layer."
  )

(defun athenacle-markdown/init-edit-server()
  (use-package edit-server
    ...
    :config
    (progn
      (setq edit-sever-defualt-mojor-mode 'markdown-mode))))

(defun athenacle-markdown/init-flymd ()
  (use-package flymd
    :defer t
    :init
    (progn
      (spacemacs/set-leader-keys "aF" 'flymd-flyit)
      (if (not athenacle-markdown/start-function)
          (athenacle-markdown/build-start-function athenacle-markdown/preview-browser))
...
(defun athenacle-markdown/post-init-smartparens ()
  (add-hook 'markdown-mode-hook 'smartparens-mode))

(defun athenacle-markdown/init-markdown-mode ()
 ...
 :mode ("\\.m[k]d" . 'markdown-mode)
    ...
    (progn
      ...
      (defun spacemacs/insert-keybinding-markdown (key)
        "Ask for a key then insert its description. Will work on both org-mode and any mode that accepts plain html."
...
(defun athenacle-markdown/init-markdown-toc ()
  (use-package markdown-toc
    :defer t
    :init (spacemacs/set-leader-keys-for-major-mode 'markdown-mode
            "it"  'markdown-toc-generate-toc)))

(defun athenacle-markdown/init-mmm-mode ()
  (use-package mmm-mode
    :commands mmm-mode
    :init (add-hook 'markdown-mode-hook (lambda() (mmm-mode t)))
    :config
    (progn
