...
(defun c-cpp-devel/load-clang-args ()
...
(defun c-cpp-devel/do-format()
  (delete-trailing-whitespace)
  (if (clang-format-buffer)
      (clang-format-buffer)))

(defun c-cpp-devel/auto-format-when-save ()
  "Format buffer when necessary."
  (defvar c-cpp-devel/auto-format-file-type (list "h" "cpp" "c" "cxx" "cc" "hpp" "c++"))
  (if (member (file-name-extension buffer-file-name) c-cpp-devel/auto-format-file-type)
          (c-cpp-devel/do-format)))

(defun c-cpp-devel/char-not-alpha-number (ch)
  (if (string-match "[a-zA-Z_0-9]" (char-to-string ch)) nil t))


(defun c-cpp-devel/clang-format-tab()
  (interactive)
  (if (c-cpp-devel/char-not-alpha-number (preceding-char))
      ;;no alpha before point, try format
      (lambda ()
        (message "tab accept. try format buffer")
        (clang-format-buffer))
    (lambda ()
      (message "tab accept. try irony-complete")
      (irony-completion-at-point))))


(defun c-cpp-devel/tab-hook()
  (add-hook 'c-mode-hook (lambda ()(define-key c-mode-map [tab] 'c-cpp-devel/clang-format-tab)))
  (add-hook 'c++-mode-hook (lambda ()(define-key c++-mode-map [tab] 'c-cpp-devel/clang-format-tab))))


(defun c-cpp-devel/clang-format-startup()
  (add-hook 'before-save-hook 'c-cpp-devel/auto-format-when-save)
  (global-set-key [C-M-l] 'clang-format-buffer)
  (c-cpp-devel/tab-hook))
